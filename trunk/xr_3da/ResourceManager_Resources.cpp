...
//		LPCSTR						target		= nullptr;
		...
		// vertex
		R_ASSERT2					(fs,cname);
        _hr = ::Render->shader_compile(name, LPCSTR(fs->pointer()), fs->length(), nullptr, &Includer, c_entry, c_target, D3DXSHADER_DEBUG | D3DXSHADER_PACKMATRIX_ROWMAJOR /*| D3DXSHADER_PREFER_FLOW_CONTROL*/, &pShaderBuf, &pErrorBuf, NULL);

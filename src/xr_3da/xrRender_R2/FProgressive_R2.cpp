...
FProgressive_R2::FProgressive_R2() : Fvisual_R2()
{
    xSWI = 0;
    last_lod = 0;
}

FProgressive_R2::~FProgressive_R2()
{

}

void FProgressive_R2::Release()
{
    Fvisual_R2::Release();
    xr_free(nSWI.sw);
    if (xSWI)		{
...
void FProgressive_R2::Load(const char* N, IReader *data, u32 dwFlags)
{
    Fvisual_R2::Load(N, data, dwFlags);
...
void FProgressive_R2::Copy(IRender_Visual *pSrc)
{
    Fvisual_R2::Copy(pSrc);
    FProgressive_R2 *pFrom = (FProgressive_R2*)pSrc;
    PCOPY(nSWI);
    PCOPY(xSWI);
}

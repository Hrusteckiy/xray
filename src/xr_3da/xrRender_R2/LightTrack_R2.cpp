...
void CROS_impl::add(xray::Light_R2* source)
{
    // Search
...
void	CROS_impl::update(IRenderable* O)
{
    ...
    // sun-tracing
    xray::Light_R2* sun = (xray::Light_R2*)RImplementation.Lights.sun_adapted._get();
    ...
    if (bTraceLights)	{
        ...
        for (u32 o_it = 0; o_it<RImplementation.lstSpatial.size(); o_it++)	{
            ...
            xray::Light_R2* source = (xray::Light_R2*)(spatial->dcast_Light());

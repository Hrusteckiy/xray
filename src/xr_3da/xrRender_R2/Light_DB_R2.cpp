...
void CLight_DB_R2::Load(IReader *fs)
{
    IReader* F = 0;
...
Light_R2* CLight_DB_R2::Create()
{
    Light_R2* L = xr_new<Light_R2>();
...
void CLight_DB_R2::add_light(Light_R2* L)
{
...
void CLight_DB_R2::Update()
{
    // set sun params
    if (sun_original && sun_adapted)
    {
        Light_R2* _sun_original = (Light_R2*)sun_original._get();
        Light_R2* _sun_adapted = (Light_R2*)sun_adapted._get();

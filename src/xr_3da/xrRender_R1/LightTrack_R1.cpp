...
void CROS_impl::add(xray::Light* source)
{
    // Search
...
void	CROS_impl::update(IRenderable* O)
{
    ...
    // sun-tracing
    xray::Light* sun = (xray::Light*)RImplementation.L_DB->sun_adapted._get();
    ...
    if (bTraceLights)	{
        ...
        for (u32 o_it = 0; o_it<RImplementation.lstSpatial.size(); o_it++)	{
            ...
            xray::Light* source = (xray::Light*)(spatial->dcast_Light());
